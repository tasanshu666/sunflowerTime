...
| 难点 | 方案 |
|---|---|
| **① S2 纯判定如何升级为完整兑换链路** | S2 的 `RedemptionService.decide()` 是**纯函数、不落库**。M2 在其**之上**新增 `RedemptionOrchestrationService`（编排层）：读模板 → 算含 K 价 → 取/建周池 → `decide()` → 落 `RedemptionRequest` → 扣账本 → 更新池 → 埋点。**不改动 `decide()` 的判定语义**，保证 S2 现有 13 条单测继续通过、成为回归基线。 |
...
| 路径 | 操作 | 说明 |
|---|---|---|
| `lib/core/constants/prd_params.dart` | [改] | 新增 `kPendingReminderHours=48`、`kPendingReminderText`、`kChildIdDefault`、`kCooldownWeeklyDefault=1`、周池/冷却相关；**保留现有 C5 全套**（T-G 做去重与 `20` 清理） |
...
| `lib/domain/services/monthly_pool_service.dart` | **[新]** | 取/建/重置周池 + C5 上限计算 |
...
| `lib/domain/repositories/monthly_pool_repository.dart` | **[新]** | 周池仓储接口 |
| `lib/core/utils/math_ext.dart` | [改] | `applyAgeTierK` 接真实 K；移除占位 `autoApproveMonthlyCap`（能力并入 `WeeklyPoolService`） |
...
| `lib/presentation/parent/pages/parent_reward_page.dart` | **[新]** | 奖励配置 + 核销 + 周池指示 |
...
| `lib/presentation/parent/widgets/pool_indicator.dart` | **[新]** | 周池用量指示 |
...
classDiagram
    ...
    RedemptionOrchestrationService --> WeeklyPoolService : 依赖
    ...
    WeeklyPoolService --> MonthlyPoolRepository : 依赖
    ...
    WeeklyPoolService ..> AgeTierParams : cap 计算
```
...
class AgeTierParams {
  ...
  final int monthlyPoolDefault;    // 周池默认（低160 / 高400）
...
**周池 key**：`weekKey(DateTime)` 采用 `'yyyy-W'`（复用现有 `core/utils/datetime_ext.dart` 的 `dayKey` 同级约定，新增 monthKey 函数或与 T-A 一并补）。
...
| `weekly_pool_reset` | `WeeklyPoolService.ensureAndReset` 发生跨月时 | pool_size, tier |
...
| **T-D** | **周池服务 + 种子数据** | `monthly_pool_service.dart`(新)、`reward_seed.dart`(新，5 条占位模板 D3/D4)、`MonthlyPoolDao`(由 T-A 提供)、`test/m2/monthly_pool_service_test.dart`、`test/m2/age_tier_params_test.dart` | T-A（硬） | **P0** |
...
```
批次 1（串行，仅 1 人）： T-A
批次 2（并行 3 人）：    T-B（埋点） ‖ T-C（编排） ‖ T-D（周池+种子）
批次 3（并行 2 人）：    T-E（商店页） ‖ T-F（家长端）
批次 4（收口 1 人）：    T-G
```

- **T-C 与 T-D 并行前提**：`WeeklyPoolService` 的接口签名以 §3.2 为准**先行冻结**（`ensureAndReset / pool / autoApproveCap / applyRedemption`），双方按契约各自写单测，集成点在 `providers.dart`（由 T-C 统一装配，避免多人改同一文件）。
...
| # | 事项 | 现状与建议 |
|---|---|---|
| **U1** | **中档（9–10 岁）的 C5 参数与周池默认值未被覆盖**：D2 只给了 K=1.2，未给 `autoApproveMaxCost` / `autoApproveCapCeiling` / `monthlyPoolDefault` / `dailyFocusCap` | **建议默认：中档全部沿用「低档」取值（50 / 40 / 160 / 90）**——理由：低档即最严档，9–10 岁暂按最严执行更安全；且**不发明新数字**，完全复用已有点，符合单点纪律。若产品后续要给中档插值（如 90/70/280），只需改 `kAgeTierParams` 一行。**请确认采用保守默认值。** |
