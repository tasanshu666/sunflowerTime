...
> ⚠️ **本文件是 M3 设计期快照，部分内容已被后续拍板覆盖，不要照它施工**（2026-09-23 加注）：
> - 本文件写的「枯萎 **7 天**」→ 现为 **3 天**（宪法 **C12**）
> - 本文件写的「付费救回 `revive()` / `kPlantReviveCostHigh/Low`」→ **已整段删除**，改**养护恢复**（宪法 C12）
> - 本文件写的「花盆自绘 `_PotPainter`」→ 现为 **PNG 美术**，且**植物图自带盆、代码不叠**（宪法 **C13**）
> - **现行口径一律以 `docs/口径裁定表_v1.md`（C1–C13）为准**；实现现状见 `docs/软件设计文档_M3M4.md`。
