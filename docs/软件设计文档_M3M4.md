...
当前 `app_database.dart` → **`int get schemaVersion => 8;`**（v8 由花谢循环引入，见 §2.3b）
...
| ② | `_ensureColumn` 补齐 M2/M3/M4 各代新增列（7 + 5 处，**v8 再加 `plants.bloomed_at`**） | 逐列幂等 |
...
| `plant` | 植物种植 / 养护 / 扩容 | `plant_plant` / `plant_water` / `plant_fertilize` / `plant_expand` / `plant_death_refund`。⚠️ `plant_revive` **已废止**（付费救回取消，宪法 C12），仅历史账本可读 |
...
| `kPlantDeathDays` / `kPlantDeathRefundRate` | 7 / 0.30 | 枯萎后再 7 天死亡 / 死亡返还 30% |
...
| 成员 | 职责 |
|---|---|
| `kGardenBackgroundSize = Size(1161, 2560)` | 背景图源尺寸（实测，勿改） |
| `gardenCoverRect(Size box)` | cover 下图片在容器内的实际绘制矩形（`scale = max(w/1161, h/2560)`，居中，允许负 origin） |
| `kGardenSignNormalizedRect` | 木牌热区（源图归一化 `0.0413, 0.6465, 0.2455, 0.7129` = 源像素 x 48~285 / y 1655~1825） |
...
`flutter test` → **412 条全绿**（`test/m2` `m3` `m4` `nav` `qa` `domain` + 根目录）。计数演进：324（接口化轮）→ 331 → 342（花园改造）→ 348（日上限）→ 360（美术链路）→ 370（花谢循环）→ 385（植物状态规则）→ 387（花园根因修复）→ 404（花园 v3）→ **412**（v3 缺陷修复 + 矮屏用例）。
...
| `test/m3/garden_layout_v3_test.dart` | 25 | v3 布局：覆盖映射纯函数独立复算、木牌坐标硬钉、**真实 `GardenPage` 渲染**（锁 2 行 / 区域内滚动 / 滚动条仅溢出可见）、加号圆心对齐盆心、三类格子底边对齐、木牌热区与花园说明弹窗 |
