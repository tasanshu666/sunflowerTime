...
| 平台 | 包名 |
|---|---|
| Android `applicationId`（`android/app/build.gradle.kts:19`） | **`com.sunflowertime.app`**（带下划线） |
| iOS `PRODUCT_BUNDLE_IDENTIFIER` | **`com.sunflowertime.app`**（camelCase） |

所有 `adb` 命令必须用 `com.sunflowertime.app`（launch / pidof / `install -r` 均以此为准）；iOS 模拟器用 `com.sunflowertime.app`。
